...
Vue.component('note-row',  {
   ...
   methods: {
      ...
      edit: function () {
         ...
         // para clonar un objeto con JQuery: $.extend({parametros}, objeto_original)
         //this.draft = $.extend({}, this.note);
         
         // para clonar con JSON
      ...
      update: function () {
         ...
         this.$http.put('/api/notes/'+this.note.id, this.draft).then(function (response) {
            this.$parent.notes.$set(this.$parent.notes.indexOf(this.note), response.note);   
            this.editing = false;
         }, function (response) {
      ...
      remove: function () {
         ...
         this.$http.delete('/api/notes/'+this.note.id, this.draft).then(function (response) {
...
var vm = new Vue({
   ...
   ready: function () {
      /* Callback con Vue */
      this.$http.get('/api/notes').then(function (response) {
